...
gen_IS_fun implements a two-stage Bayesian Markov Chain Monte Carlo (MCMC) sampling framework for models with not fully observed covarite $\zeta$ using Importance Sampling (IS) and Copula-based approximations. 
The function supports four sampling algorithms specified via the model parameter:

* "plugin": Fixes $\zeta$ to its Stage 1 posterior mean (or other point estimate) across all iterations.
...
* **zeta_ppd**: A numeric matrix of dimension `S × n` containing `S` posterior predictive draws of unobserved variable $\zeta$ obtained from Stage 1. For "plugin", may also be a numeric vector of length `n` containing a point estimate of $\zeta$ obtained from Stage 1.
