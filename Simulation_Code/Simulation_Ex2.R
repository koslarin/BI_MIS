...
z1_vec_Cor0.3_merg_mod = matrix(NA, nrow = Runs, ncol = n)
...
for (i in 1:Runs) {
  ...
  y_vec_Cor0.3_merg_mod[i,] <- b0  + b_z*zeta + rnorm(n, sd = sigma_y)
  z1_vec_Cor0.3_merg_mod[i,] <- zeta + mvrnorm(n=1, mu = rep(0, times=n), Sigma =z_cov_matrix)
}
...
for (t in 1:Runs) {
  y = y_vec_Cor0.3_merg_mod[t,]
  z1 = z1_vec_Cor0.3_merg_mod[t, ]
  ...
  for (k in 1:N) {
    ...
    Stage1_Cor0.3_zeta_ppd[t,k,] <- zeta_part
  }
}
